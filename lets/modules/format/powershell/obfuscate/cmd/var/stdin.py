...
class Stdin(DockerExtension, Module):
    """
    Format a powershell script into an obfuscated cmd.exe command by passing
    the command content through stdin and an environment variable.
    """
    FLAGS = [
        "None",
        "NoExit",
    ...
    def do(self, data:bytes=None, options:dict=None) -> bytes:
        ...
        cmd = "Invoke-Obfuscation -Quiet -ScriptPath /data/in -Command 'LAUNCHER,STDIN++,%s' | Out-File /data/out" % (flags if flags else "0")
