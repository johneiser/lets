...
class Var(DockerExtension, Module):
    """
    Format a powershell script into an obfuscated cmd.exe command by 
    setting and using an environment variable for both the command
    content and the powershell executable.
    """
    FLAGS = [
        "None",
        "NoExit",
    ...
    def do(self, data:bytes=None, options:dict=None) -> bytes:
        ...
        cmd = "Invoke-Obfuscation -Quiet -ScriptPath /data/in -Command 'LAUNCHER,VAR++,%s' | Out-File /data/out" % (flags if flags else "0")
